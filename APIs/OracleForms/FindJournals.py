...
class FindJournals(OracleForms):
    def __init__(self, *args, **kwargs):
        OracleForms.__init__(self, *args, **kwargs)
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_find_journals_elements.json")

    def check_find_journals_screen_exists(self, 
                                          timeout: int) -> bool:
        """
        Checks if the 'Find Journals' screen exists
        ...
        try:
            ...
            element_exists = self.wait_until_element_exists(
                oracle_forms_elements["window_find_journals"], timeout
            )
        ...
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Find Journals' screen"
                )
            element_exists = self.wait_until_element_exists(oracle_forms_elements['window_find_journals'], std_15s_wait)
            if(not element_exists):
               raise Exception("Cannot find 'Find Journals' screen")
        ...
        try:
            ...
            if field_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + field_name
                    + " doesn't exists in 'Find Jourmals' screen"
                )
            self.java.select_window(
            title=window_title, bring_foreground=True, timeout=std_15s_wait
            )
    ...
    def write_field_by_name(self, 
        ...
        try:
            ...
            self.write_in_element(oracle_forms_elements[field_name], value, verify)
        except Exception as ex:
            raise ex
