...
class DiscreteJobs(OracleForms):
    def __init__(self, *args, **kwargs):
        OracleForms.__init__(self, *args, **kwargs)
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_discrete_jobs.json")


    def check_discrete_jobs_screen_exists(self, timeout: int):
        element_exists = False
        try:
            oracle_forms_elements = self.oracle_forms_elements
            element_exists = self.wait_until_element_exists(
                oracle_forms_elements["window_discrete_jobs"], timeout
            )
        ...
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Discrete Jobs' screen"
                )
