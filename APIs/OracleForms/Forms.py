...
class Forms(OracleForms):
    def __init__(self, *args, **kwargs):
        ...
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_forms_elements.json")
        self.error_element_locator = "role:internal frame"
        self.error_element_attribute = "name"
    
    def check_forms_screen_exists(self, timeout: int):
        ...
        try:
            ...
            element = self.get_element_by_attribute(
                ...
                oracle_forms_elements["window_forms"], timeout
            )
    ...
    def click_button_by_name(self, window_title: str, button_name: str):
        """
        Clicks a button in 'Forms' screen by it's name
        ...
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Forms' screen"
                )
            element = self.get_element_by_attribute(
                self.error_element_locator, self.error_element_attribute,
                oracle_forms_elements["window_forms"], std_15s_wait
            )
            if element is None:
               raise Exception("Cannot find 'Forms' screen")
