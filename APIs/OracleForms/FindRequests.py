...
class FindRequests(OracleForms):
    def __init__(self, *args, **kwargs):
        OracleForms.__init__(self, *args, **kwargs)
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_find_requests_elements.json")

    def check_find_requests_screen_exists(self, timeout: int):
        element_exists = False
        try:
            oracle_forms_elements = self.oracle_forms_elements
            element_exists = self.wait_until_element_exists(
                oracle_forms_elements["window_find_requests"], timeout
            )
    ...
    def click_button_by_name(self, window_title: str, button_name: str):
        """
        Clicks a button in 'Find Requests' screen by it's name
        @param window_title: The Oracle Forms application window title name
        @param button_name: Must be one of this possible values:
        "button_submit_a_new_request", 
        "button_all_my_requests",
        "button_specific_requests", 
        "button_find", 
        "button_close_form"
        """
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Find Requests' screen"
                )
            element_exists = self.wait_until_element_exists(oracle_forms_elements['window_find_requests'], std_15s_wait)
            if(not element_exists):
               raise Exception("Cannot find 'Find Requests' screen")
    ...
    def write_input_by_name(self, input_name: str, input_value: str):
        """
        Writes a custom value inside an element in 'Find Requests' screen by it's name
