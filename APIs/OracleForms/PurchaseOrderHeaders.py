...
class PurchaseOrderHeaders(OracleForms):
    def __init__(self, *args, **kwargs):
        OracleForms.__init__(self, *args, **kwargs)
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_purchase_order_headers_elements.json")

    def check_purchase_order_headers_screen_exists(self, timeout: int):
        element_exists = False
        try:
            oracle_forms_elements = self.oracle_forms_elements
            element_exists = self.wait_until_element_exists(
                oracle_forms_elements["window_purchase_order_headers"], timeout
            )
        ...
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Purchase Order Headers' screen"
                )
            element_exists = self.wait_until_element_exists(oracle_forms_elements['window_purchase_order_headers'], std_15s_wait)
            if(not element_exists):
               raise Exception("Cannot find 'Purchase Order Headers' screen")
