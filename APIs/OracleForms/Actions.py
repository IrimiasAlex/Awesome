...
class Actions(OracleForms):
    def __init__(self, *args, **kwargs):
        OracleForms.__init__(*args, **kwargs)
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_actions_elements.json")
            
    def check_actions_screen_exists(self, timeout: int):
        element_exists = False
        try:
            oracle_forms_elements = self.oracle_forms_elements
            element_exists = self.wait_until_element_exists(
                oracle_forms_elements["window_actions"], timeout
            )
        ...
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Actions' screen"
                )
            element_exists = self.wait_until_element_exists(oracle_forms_elements['window_actions'], std_15s_wait)
            if(not element_exists):
               raise Exception("Cannot find 'Actions' screen")
    ...
    def write_input_by_name(self, input_name: str, input_value: str):
        """
        Writes a custom value inside an element in 'Actions' screen by it's name
