...
class Note(OracleForms):
    def __init__(self, *args, **kwargs):
        OracleForms.__init__(self, *args, **kwargs)
        self.oracle_forms_elements = self.actions.read_configuration("data/oracle_forms_config/oracle_note_elements.json")
        self.error_element_locator = "role:internal frame"
        self.error_element_attribute = "name"
    
    def check_note_screen_exists(self, timeout: int):
        ...
        try:
            ...
            element = self.get_element_by_attribute(
                ...
                oracle_forms_elements["window_note"], timeout
            )
    ...
    def click_button_by_name(self, window_title: str, button_name: str):
        """
        Clicks a button in 'Note' screen by it's name
        ...
        try:
            ...
            if button_name not in oracle_forms_elements:
                raise Exception(
                    ...
                    + button_name
                    + " doesn't exists in 'Note' screen"
                )
            element = self.get_element_by_attribute(
                self.error_element_locator, self.error_element_attribute,
                oracle_forms_elements["window_note"], std_15s_wait
            )
            if element is None:
               raise Exception("Cannot find 'Note' screen")
